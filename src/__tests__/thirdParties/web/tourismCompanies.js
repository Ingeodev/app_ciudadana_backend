...
describe("Web - Tourism Companies management API points: ", () => {
  ...
  describe("Create a (test) tourism category. ", () => {
  ...
  describe("POST /tourism_company/ ", () => {
    test("Should respond with status 201 and the new object (data) after creating a new company.", async () => {
    ...
    test("Should fail with status 404 and an error with a message if category is not found.", async () => {
      const response2 = await request(usedHost)
        .post("/")
        .set(requestHeaders)
        .send({
      ...
      expect(response2.body).not.toHaveProperty("meta");
      expect(response2.body).not.toHaveProperty("data");
      expect(response2.body).toHaveProperty("status", 404);
      expect(response2.body).toHaveProperty("code");
      expect(response2.body).toHaveProperty("detail");
    });
  });

  describe("POST /tourism_company/api_key ", () => {
  ...
  describe("GET /tourism_company/api_key/:id ", () => {
  ...
  describe("GET /tourism_company/:id ", () => {
    test("Should respond with status 200 and one company object created.", async () => {
      const response0 = await request(usedHost)
        .get(`/${testCompany0.id}`)
        .set(requestHeaders);
      expect(response0.statusCode).toBe(200);
  ...
  describe("GET /tourism_company/ ", () => {
    ...
    test("Disabled - Should respond with status 200 and an empty array, because there are no companies registered.", async () => {
      // const response0 = await request(usedHost)
      //   .get("/")
      //   .set(requestHeaders)
      //   .query({ page: { number: 1, size: 2 } });
  ...
  describe("POST /tourism_company/edit ", () => {
  ...
  describe("POST /tourism_company/delete ", () => {
    test("Should respond with status 200 and the edited object (data).", async () => {
      const response0 = await request(usedHost)
        .post("/delete")
        .set(requestHeaders)
  ...
  describe("Delete the (test) tourism category created. ", () => {
    test("Should respond with status 200 and the tourism category id deleted.", async () => {
      const response0 = await request(`${global.thirdPartiesMicroserviceDefaultHost}/api/web/v1/third_parties/tourism_categories`)
        .post("/delete")
        .set(requestHeaders)
        .send({
